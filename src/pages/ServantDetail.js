  ...
  useEffect(() => {
    const fetchServantDetails = async () => {
      try {
        ...
        if (foundServant) {
          console.log('Found Servant:', foundServant); //buat semacam debugging
          setServant(foundServant);
        } else {
          console.log(`Servant with ID ${servantId} not found`); //buat semacam debugging
          setServant(null);
        }
      } catch (error) {
    ...
    //menampilkan detail servant yang datanya di fetch dari API berdasarkan ID yang dipilih pada halaman Servant
    fetchServantDetails();
  }, [servantId]);

  if (!servant) {
    return <div className="ServantDetail">Loading...</div>; //buat keterangan loading
  }

  //memeriksa gambar servant dari ascend 1 sampai 3
